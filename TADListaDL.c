...
La implementación del presente código se realiza mediante el principio de
"Lista Doblemente Ligada", i.e., nodos que contienen un elemento y se
encuentran ligados tanto hacia su siguiente como hacia su anterior.
...
a nivel de bytes.
*/
posicion Search(lista *l, char word[])
{
	posicion p;
	...
	for (p = l->frente; p != NULL; p = p->siguiente)
	{
...
Descripción: Recibe una lista y un índice (entre 1 y el tamaño de la lista) y devuelve el elemento que se encuentra
en la lista en ese índice partiendo del frente de este =1 hacia atrás.
...
Descripción: Recibe una lista y un índice (entre 1 y el tamaño de la lista) y devuelve la posicion del elemento que se
...
void Insert(lista *l, posicion p, elemento e, boolean b)
{
	...
	if (ValidatePosition(l, p))
	{
		...
		else // Atras de p
		{
			...
			else // Si p no era el final de la lista
			{
				p->siguiente = aux;
				aux->anterior = p;
				aux->siguiente = aux2;
